...
export class SmartContractRepository extends BaseRepository implements ISmartContractRepository {
    ...
    async insertExecuteContract(from: string, contract: string, functionName: string, parameters: string, gas: number, fee: number, status: string, typeUrl: string, internalChainId: number, denom: string, accountNumber: number, sequence: string, safeId: number): Promise<any> {
        let contractTx = new SmartContractTx();
    ...
    async checkExistSmartContractTx(smartContractTxId: number, internalChainId: number): Promise<any> {
    ...
    async validateTxBroadcast(transactionId: number): Promise<any> {
        let smartContractTx = await this.findOne({
