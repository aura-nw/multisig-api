...
import { IGeneralRepository, IMultisigConfirmRepository, IMultisigTransactionsRepository, IMultisigWalletOwnerRepository, IMultisigWalletRepository } from "src/repositories";
...
import { Chain, SmartContractTx } from "src/entities";
...
export class SmartContractService extends BaseService implements ISmartContractService {
    ...
    async createExecuteMessage(request: MODULE_REQUEST.ExecuteMessageRequest): Promise<ResponseDto> {
        ...
        try {
            ...
            let safe = await this.safeRepos.findOne({ where: { safeAddress: request.senderAddress, internalChainId: request.internalChainId } });

            // Safe data into DB
            let transactionResult = await this.repos.insertExecuteContract(request.senderAddress, request.contractAddress, request.functionName, JSON.stringify(request.param), request.gasLimit, request.fee, TRANSACTION_STATUS.AWAITING_CONFIRMATIONS, NETWORK_URL_TYPE.EXECUTE_CONTRACT, request.internalChainId, signResult.denom, signResult.accountNumber, signResult.sequence.toString(), safe.id);
    ...
    async confirmExecuteMessage(
        ...
        console.log(request)
        try {
            let transaction = await this.repos.checkExistSmartContractTx(request.transactionId, request.internalChainId);

            const validateOwner = await this.multisigConfirmRepos.validateOwner(request.fromAddress, transaction.fromAddress, request.internalChainId);
            console.log(validateOwner)

            //User has confirmed transaction before
            const userHasSigned = await this.multisigConfirmRepos.checkUserHasSigned(request.transactionId, request.fromAddress);
            ...
            await this.repos.validateTransaction(request.transactionId, request.internalChainId);
    ...
    async rejectExecuteMessage(request: MODULE_REQUEST.RejectTransactionParam): Promise<ResponseDto> {
        const res = new ResponseDto();
        try {
            //Check status of multisig transaction when reject transaction
            let transaction =
                await this.repos.checkExistSmartContractTx(request.transactionId, request.internalChainId);
    ...
    async sendExecuteMessage(request: MODULE_REQUEST.SendTransactionRequest): Promise<ResponseDto> {
        const res = new ResponseDto();
        try {
            let chain = await this.chainRepos.findOne({ where: { id: request.internalChainId } });
            ...
            let smartContractTx = await this.repos.validateTxBroadcast(request.transactionId);

            //Validate owner
            await this.multisigConfirmRepos.validateOwner(request.owner, smartContractTx.fromAddress, request.internalChainId);

            //Make tx
            let txBroadcast = await this.makeTx(request.transactionId, smartContractTx);

            try {
                await this.multisigConfirmRepos.insertIntoMultisigConfirmContractType(request.transactionId, request.owner, '', '', request.internalChainId, MULTISIG_CONFIRM_STATUS.SEND);
                ...
                if (typeof error.txId === 'undefined') {
                    ...
                    await this.repos.updateTxBroadcastSucces(smartContractTx.id, error.txId);
